...
## About
sentimentalstock is an open source stock market analysis software that uses Elasticsearch to store Twitter and news headlines data for stocks. sentimentalstock analyzes the emotions of what the author writes and does sentiment analysis on the text to determine how the author "feels" about a stock. It could be used for more than finding sentiment of just stocks, it could be used to find sentiment of anything...
...
1) Download/clone sentimentalstock repo with git.
2) Set up sentimentalstock, elasticsearch and kibana containers using Docker compose
```
cd sentimentalstock
cp config.py.sample config.py
***see how to use below for config.py (sentimentalstock config) changes***
docker-compose build && docker-compose up
```
**This will volume mount config.py (sentimentalstock settings) and twitteruserids.txt to those files in your local git cloned "sentimentalstock" directory**

3) Once all the containers have started up, shell into the container

`docker exec -it sentimentalstock_sentimentalstock_1 bash`

4) See examples below for running sentimentalstock.
...
2) Copy config.py.sample to config.py (sentimentalstock config file)
...
Run sentiment.py to create 'sentimentalstock' index in Elasticsearch and start mining and analyzing Tweets using keywords and the stock symbol TSLA
...
Run stockprice.py to add stock prices to 'sentimentalstock' index in Elasticsearch
...
Load 'sentimentalstock' index in Kibana. For index pattern you can use 'sentimentalstock' if you only have the single index or 'sentimentalstock-*', etc. For time-field name you will want to use the date/time field 'date'.
...
optional arguments:
  ...
  -i INDEX, --index INDEX
                        Index name for Elasticsearch (default: sentimentalstock)
